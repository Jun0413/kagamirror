...
function displayHeadlines() {
    ...
    headlines.displayHeadlines((newsAgency, headlineInfo) => {
        if (headlineInfo) {
            $(".newsheadline").fadeOut('fast', function() {
                $(this).text(headlineInfo.title).fadeIn('fast');
            });
            $(".newsagency").fadeOut('fast', function() {
                $(this).text(newsAgency + " " + moment(new Date(headlineInfo.pubdate)).fromNow()).fadeIn('fast');
            });
        }
    });
}
...
ipc.on("display-school-events", _ => {
    ...
    displaySchoolEvents(5, 5 * 1000); // set # of events displayed & scrollInterval
});
